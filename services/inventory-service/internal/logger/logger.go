...
func log(level, message string, fields Fields) {
	...
	entry := map[string]any{"timestamp": time.Now().UTC().Format(time.RFC3339Nano), "level": level, "service": getenv("SERVICE_NAME", "inventory-service"), "environment": getenv("APP_ENV", "development"), "message": message}
